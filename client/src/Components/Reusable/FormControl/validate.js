const validate = (values) => {
    ...
    } else if (!/^[0-9]+$/.test(values.hp)) {
        errors.hp = 'El campo debe ser númerico'
    };

    if(!values.atk) {
        errors.atk = 'El campo esta vacio'
    } else if (!/^[0-9]+$/.test(values.hp)) {
        errors.atk = 'El campo debe ser númerico'
    };

    if(!values.def) {
        errors.def = 'El campo esta vacio'
    } else if (!/^[0-9]+$/.test(values.def)) {
        errors.def = 'El campo debe ser númerico'
    };

    if(!values.spc_atk) {
        errors.spc_atk = 'El campo esta vacio'
    } else if (!/^[0-9]+$/.test(values.spc_atk)) {
        errors.spc_atk = 'El campo debe ser númerico'
    };

    if(!values.spc_def) {
        errors.spc_def = 'El campo esta vacio'
    } else if (!/^[0-9]+$/.test(values.spc_def)) {
    ...
    if(!values.spd) {
        ...
        errors.spd = 'El campo debe ser númerico'
    };
